	...
	for (int lop = 0; lop < 100; ++lop)
	{
	    ...
	    {    for (int j =1; j<N-1; j++)
	        {
	            ...
	            u_new[i][j]=u_k[i][j]+0.25*(dtau[i][j]*rus[i][j]);
	            v_new[i][j]=v_k[i][j]+0.25*(dtau[i][j]*rvs[i][j]);
	        }
	    }
	    BC(N,u_new,v_new,p_new,dummyu);			// BC after first step RK
	    
	    RHS(N,JC,u_new,v_new,p_new,zx,ey,ex,zy,Re,eps,ep,rcs,rus,rvs,rho1,rho2); 
	    ...
	    for (int i =1; i<N-1; i++)			// Second step of RK
	    {
	        for (int j =1; j<N-1; j++)
	        {
	            ...
	            u_new[i][j]=u_k[i][j]+0.33*(dtau[i][j]*rus[i][j]);
	            v_new[i][j]=v_k[i][j]+0.33*(dtau[i][j]*rvs[i][j]);
	        }
	    }
	    BC(N,u_new,v_new,p_new,dummyu);			// BC after second step RK
	    ...
	    for (int i =1; i<N-1; i++)			// Third step of RK
	    {
	        ...
	        {   
	            ...
	            u_new[i][j]=u_k[i][j]+0.5*(dtau[i][j]*rus[i][j]);
	            v_new[i][j]=v_k[i][j]+0.5*(dtau[i][j]*rvs[i][j]);
	        }
	    }
	    BC(N,u_new,v_new,p_new,dummyu);			// BC after third step RK
